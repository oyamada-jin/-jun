...
function addIntro(flag) {
    ...
    if (flag==0) { //テキストボックスを追加する
        ...
        newIntroFile.setAttribute('onchange',"handleFileSelectIntro('project_intro_file_"+introCount+"','project_intro_img_"+introCount+"')");
        ...
        newIntroThumbnail.setAttribute('onclick',"document.getElementById('project_intro_file_"+introCount+"').click()");
    ...
    if (introCount >= 2) {
        ...
        if (deleteElement) {
            ...
            for (let i = deleteId;  i <= introCount; ++i) {
                ...
                //判別タグの変更
                let introFlag = document.getElementById("project_intro_flag_"+i);
                if (introFlag.value=0) { //テキストボックスなら
                    
                }else if(introFlag ==1){ //画像なら
                    // 画像アップロードのfileタイプinputタグ変更
                    let introFile = document.getElementById("project_intro_content_" + i);
                    if (introFile) {
                        introFile.id = "project_intro_content_" + (i - 1);
                        introFile.setAttribute('onchange', "handleFileSelectIntro('project_intro_file_" + (i - 1) + "','project_intro_img_" + (i - 1) + "')");
                    }

                    // imgタグの変更
                    let introThumbnaiil = document.getElementById("project_intro_img_" + i);
                    if (introThumbnaiil) {
                        introThumbnaiil.id = "project_intro_img_" + (i - 1);
                        introThumbnaiil.setAttribute('onclick', "document.getElementById('project_intro_file_" + (i - 1) + "').click()");
                    }    
                }
                ...
                //Pタグの変更
                let DeleteP = document.getElementById("DeleteIntroP_" + i);
                if (DeleteP) {
                    DeleteP.id = "DeleteP_" + (i - 1);
                    DeleteP.setAttribute('onclick', "deleteIntro(" + (i - 1) + ")");
                }
            }
